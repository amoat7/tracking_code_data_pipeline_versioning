...
def run_pipeline(steps):
    ...
    with mlflow.start_run(run_name='pipeline', nested=True) as active_run:
        ...
        if "train_model" in active_steps:
            training_model_run = mlflow.run("/", "train_model", parameters={"data_path": file_path_uri})
        ...
        if "register_model" in active_steps:
            if training_model_run_id is not None and training_model_run_id!= 'None':
                register_model_run = mlflow.run("/", "register_model", parameters={"mlflow_run_id": training_model_run_id})
